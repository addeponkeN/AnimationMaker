    ...
    public class Screen
    {
        ...
        public virtual void Update(GameTime gameTime, bool otherScreenHasFocus, bool coveredByOtherScreen)
        {
            ...
            for(int i = 0; i < Popups.Count; i++)
            {
                ...
                pup.Update(gameTime, otherScreenHasFocus, coveredByOtherScreen);
            }
            ...
            if(IsExiting)
            {
                ...
                if(UpdateTransition(gameTime, TransitionOffTime, 1) == 0)
                {
            ...
            else if(coveredByOtherScreen)
            {
                ...
                if(UpdateTransition(gameTime, TransitionOffTime, 1) == 1)
                {
                    // Still busy transitioning.
                    ScreenState = ScreenState.TransitionOff;
                }
                else if(UpdateTransition(gameTime, TransitionOnTime, 1) == 2)
                {
            ...
            {
                if(UpdateTransition(gameTime, TimeSpan.FromSeconds(0.25), 1, true) == 1)
                {
                    ScreenState = ScreenState.TransitionOn;
                }
                else
            ...
            else
            {
                ...
                if(UpdateTransition(gameTime, TransitionOnTime, -1) == 1)
                {
                    // Still busy transitioning.
                    ScreenState = ScreenState.TransitionOn;
                }
        ...
        /// </summary>
        int UpdateTransition(GameTime gameTime, TimeSpan time, int direction, bool pause = false)
        {

            foreach(var screen in ChildScreens)
            {
                screen.UpdateTransition(gameTime, time, direction);
            }
